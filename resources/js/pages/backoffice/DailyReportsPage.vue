...
import {
    AppCreateResource, AppRemoveResource,
...
const handleAction = (action, model) => {
    if (action === 'edit' && model?.edit_url) {
        window.location.href = model.edit_url;
        return;
    }
                ...
                <TableListTemplate v-if="!loading">
                    ...
                    <template v-slot:table>
                        <AppTable
                            ...
                            @actionDispatched="handleAction($event.action, $event.model)"
                        />
                    </template>
