...
final class UpdateResourceTool implements Tool
{
    public function __construct(
        private readonly int $userId,
        private readonly UpdatableResource $strategy,
    ) {}
        ...
        return array_merge(
            [
                'resource' => $schema->string()->enum([$this->strategy->resourceName()])
                    ->description('Tipo de recurso a actualizar.')->required(),
                'id' => $schema->integer()
                    ->description('ID del recurso a actualizar.')->required(),
            ],
    ...
    public function handle(Request $request): Stringable|string
    {
        ...
        return $this->strategy->update($this->userId, (int) $request['id'], $data);
    }
}
