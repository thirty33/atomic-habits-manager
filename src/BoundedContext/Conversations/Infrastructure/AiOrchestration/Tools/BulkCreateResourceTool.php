...
final class BulkCreateResourceTool implements Tool
{
    public function __construct(
        private readonly int $userId,
        private readonly BulkCreatableResource $strategy,
    ) {}
    ...
    public function schema(JsonSchema $schema): array
    {
        ...
        return [
            'resource' => $schema->string()->enum([$this->strategy->resourceName()])
                ->description('Tipo de recurso a crear.')->required(),
            'items' => $schema->array()
    ...
    public function handle(Request $request): Stringable|string
    {
        ...
        return $this->strategy->bulkCreate($this->userId, $items);
    }
}
