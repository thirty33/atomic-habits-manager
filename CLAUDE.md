...
- `backoffice-module-creation` — Create new backoffice modules following the established architecture — jsonGroup routes, ViewModel pattern, Blade + Vue bridge, sidebar entry, and global component registration.
- `cloudflare-tunnel` — Create temporary Cloudflare quick tunnels to expose the local application via a public HTTPS URL, without authentication or Cloudflare account.
- `laravel-ai-development` — Build AI agents and tools with laravel/ai SDK, including tool creation, agent configuration, and provider setup (DeepSeek, OpenAI, etc.).
- `laravel-sail-mcp-setup` — Configure Laravel Boost and MCP servers to work correctly with Docker and Laravel Sail, including the stdio/TTY fix for Claude Code.
- `mcp-server-development` — Build MCP (Model Context Protocol) servers and tools with laravel/mcp to expose functionality to external AI clients like Claude Code.
